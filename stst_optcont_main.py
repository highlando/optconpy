...
#                              'NewtonIt{0}').format(newtk))
    ...
    pdatstr = get_datastr(nwtn=None, time=None, meshp=N, timps=tip)
    try:
        Z = dou.load_npa(ddir + pdatstr + cntpstr + '__Z')
    except IOError:
        Z = pru.proj_alg_ric_newtonadi(mmat=M, fmat=-A,
                                       jmat=stokesmatsc['J'],
                                       bmat=tb_mat, wmat=tct_mat,
                                       nwtn_adi_dict=tip['nwtn_adi_dict']
                                       )['zfac']
        dou.save_npa(Z, fstring=ddir + pdatstr + cntpstr + '__Z')
    ...
    for t in np.linspace(tip['t0']+DT, tip['tE'], Nts):
        ...
        rhsn = M*old_v + DT*(fvn + tb_mat * (tb_mat.T * wft))
        ...
        #TODO: rhsp!!!
        vpn, sadlu = lau.solve_sadpnt_smw(amat=amat, jmat=stokesmatsc['J'],
                                          rhsv=rhsn, sadlu=sadlu,
                                          umat=-DT*tb_dense, vmat=mtxtb.T,
                                          return_alu=True)
        old_v = vpn[:NV]
