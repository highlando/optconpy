...
class OptConPyFunctions(unittest.TestCase):
    ...
    def test_ystarvec(self):
        ...
        to, tt = 0, 3
        ystveco = contp.ystarvec(to)
        ystgueo = np.vstack([to * np.ones((contp.NY, 1)),
                            np.ones((contp.NY, 1))])

        ystvect = contp.ystarvec(tt)
        ystguet = np.vstack([tt * np.ones((contp.NY, 1)),
                            np.ones((contp.NY, 1))])

        self.assertTrue(np.allclose(ystveco, ystgueo))
        self.assertTrue(np.allclose(ystvect, ystguet))
